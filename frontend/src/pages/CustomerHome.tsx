  ...
  return (
    <main className="min-h-screen bg-[#f7f8fc] pb-24 text-slate-950">
      <div className="mx-auto max-w-6xl px-3 py-3 sm:px-6 sm:py-6">
        ...
        <section className="mt-6"><div className="mb-3 flex items-end justify-between"><div><h2 className="text-lg font-black">Stores near you</h2><p className="text-[10px] font-semibold text-slate-500">Verified neighbourhood merchants</p></div><button onClick={() => chooseMode('all')} className="flex items-center gap-1 text-[10px] font-black text-blue-600">View all ({visibleStores.length}) <ChevronRight className="h-3 w-3" /></button></div>{error && <p className="mb-3 rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700">{error}</p>}{loading && <p className="py-8 text-center text-xs font-bold text-slate-500">Stores load ho rahe hain...</p>}{!loading && !error && visibleStores.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-xs font-bold text-slate-500">Is selection me store nahi mila.</div>}<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visibleStores.map((store) => <Link key={store.id} to={`/s/${store.slug}`} className="rounded-2xl border border-slate-100 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex gap-3"><div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100">{store.logo ? <img src={store.logo} alt="" className="h-full w-full object-cover" /> : <StoreIcon className="h-6 w-6 text-slate-400" />}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><h3 className="truncate text-sm font-black">{store.name}</h3><p className="text-[10px] font-bold uppercase text-slate-500">{store.business_type || 'Local store'}</p></div>{store.distance_km !== undefined && <span className="shrink-0 rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black text-blue-700">{store.distance_km} km</span>}</div><p className="mt-1 flex items-center gap-1 truncate text-[10px] font-medium text-slate-500"><MapPin className="h-3 w-3 shrink-0" />{store.address || 'Local store'}</p><p className="mt-2 text-[10px] font-black text-emerald-600">● Open now · Fast local delivery</p></div></div><div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2"><span className="text-[10px] font-bold text-slate-500">{store.allow_home_delivery ? 'Free delivery options' : 'Store pickup available'}</span><span className="rounded-lg bg-blue-600 px-3 py-1.5 text-[10px] font-black text-white">Visit Store →</span></div></Link>)}</div></section>

        <section className="mt-6"><div className="mb-3 flex items-end justify-between"><div><h2 className="text-lg font-black">Recommended for you</h2><p className="text-[10px] font-semibold text-slate-500">Daily essentials from nearby stores</p></div><span className="rounded-full bg-blue-50 px-2 py-1 text-[9px] font-black uppercase text-blue-700">Top picks</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4">{recommendations.map((store) => <Link key={`recommend-${store.id}`} to={`/s/${store.slug}`} className="rounded-2xl bg-white p-3 shadow-sm"><div className="flex h-28 items-center justify-center overflow-hidden rounded-xl bg-slate-100">{store.logo ? <img src={store.logo} alt="" className="h-full w-full object-cover" /> : <StoreIcon className="h-8 w-8 text-slate-400" />}</div><p className="mt-2 truncate text-[9px] font-black uppercase text-slate-500">{store.business_type || 'Local store'}</p><h3 className="mt-1 truncate text-xs font-black">{store.name}</h3><p className="mt-2 text-[10px] font-black text-blue-600">Open store +</p></Link>)}</div></section>

        <section className="mt-6 rounded-2xl bg-blue-50 p-4 text-center"><p className="text-[10px] font-black text-slate-700">♢ Apani Dukan Assurance</p><div className="mt-3 grid grid-cols-3 gap-2 text-[9px] font-bold text-slate-700"><span className="rounded-xl bg-white p-2">▣<br />Direct Local<br />Shops</span><span className="rounded-xl bg-white p-2">⌁<br />Kirana Price<br />Match</span><span className="rounded-xl bg-white p-2">₹<br />Cash / UPI at<br />Door</span></div></section>
      </div>
