...
import { ChevronRight, LocateFixed, MapPin, Store as StoreIcon } from 'lucide-react'
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-[#f7f8fc] pb-24 text-slate-950">
      <div className="mx-auto max-w-6xl px-3 py-3 sm:px-6 sm:py-6">
        ...
        <section className="mt-6 min-w-0 overflow-hidden"><div className="mb-3 flex items-end justify-between gap-2"><div className="min-w-0"><h2 className="text-lg font-black">Stores near you</h2><p className="text-[10px] font-semibold text-slate-500">A quick preview of nearby merchants</p></div><Link to="/customer-stores" className="flex shrink-0 items-center gap-1 text-[10px] font-black text-blue-600">Explore all ({stores.length}) <ChevronRight className="h-3 w-3" /></Link></div>{error && <p className="mb-3 rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700">{error}</p>}{loading && <p className="py-8 text-center text-xs font-bold text-slate-500">Stores load ho rahe hain...</p>}{!loading && !error && storePreview.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-xs font-bold text-slate-500">Is selection me store nahi mila.</div>}<div className="grid gap-3 sm:grid-cols-2">{storePreview.map((store) => <Link key={store.id} to={`/s/${store.slug}`} state={{ returnTo: '/customer-home' }} className="min-w-0 overflow-hidden rounded-2xl border border-slate-100 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex min-w-0 gap-3"><div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100">{store.logo ? <img src={store.logo} alt="" className="h-full w-full object-cover" /> : <StoreIcon className="h-6 w-6 text-slate-400" />}</div><div className="min-w-0 flex-1"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><h3 className="truncate text-sm font-black">{store.name}</h3><p className="truncate text-[10px] font-bold uppercase text-slate-500">{store.business_type || 'Local store'}</p></div>{store.distance_km !== undefined && <span className="shrink-0 rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black text-blue-700">{store.distance_km} km</span>}</div><p className="mt-1 flex min-w-0 items-center gap-1 truncate text-[10px] font-medium text-slate-500"><MapPin className="h-3 w-3 shrink-0" /><span className="truncate">{store.address || 'Local store'}</span></p><p className="mt-2 truncate text-[10px] font-black text-emerald-600">● Open now · Fast local delivery</p></div></div><div className="mt-3 flex min-w-0 items-center justify-between gap-2 border-t border-slate-100 pt-2"><span className="min-w-0 truncate text-[10px] font-bold text-slate-500">{store.allow_home_delivery ? 'Delivery available' : 'Store pickup available'}</span><span className="shrink-0 rounded-lg bg-blue-600 px-3 py-1.5 text-[10px] font-black text-white">Visit Store →</span></div></Link>)}</div></section>
