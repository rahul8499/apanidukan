...
export default function CustomerOrdersHome() {
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fc] pb-24 text-slate-900">
      <div className="mx-auto max-w-6xl px-3 py-3 sm:px-6 sm:py-6">
        <CustomerAppHeader subtitle="Your orders" />
        <div className="mb-5 rounded-3xl bg-gradient-to-r from-blue-600 via-indigo-600 to-slate-900 p-5 text-white shadow-lg">
          <div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/15"><Package className="h-6 w-6" /></span><div><p className="text-[9px] font-black uppercase tracking-widest text-blue-100">Order centre</p><h1 className="mt-0.5 text-xl font-black">My Orders</h1><p className="mt-0.5 text-xs text-blue-100">Track purchases from every Apani Dukan store.</p></div></div>
        </div>
        {!token ? (
        ...
        ) : <section className="space-y-3">{orders.length === 0 && <div className="rounded-2xl bg-white p-8 text-center text-sm font-bold text-slate-500 shadow-sm">No orders found for this mobile number.</div>}{orders.map(order => <Link key={order.reference} to={`/s/${order.store_slug}/order/${order.reference}?token=${order.tracking_token}`} className="block rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition active:scale-[0.99]"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span className="rounded-xl bg-blue-50 p-2 text-blue-600"><Store className="h-4 w-4" /></span><span className="truncate font-black">{order.store_name}</span></div><span className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-500">#{order.reference}</span></div><div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-sm"><strong>₹{Number(order.total).toFixed(2)}</strong><span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black uppercase text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" />{order.status}</span></div></Link>)}</section>}
      </div>
