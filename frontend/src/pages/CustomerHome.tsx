...
import { ChevronRight, ClipboardList, Grid2X2, Home, LocateFixed, MapPin, Search, ShoppingBag, Store as StoreIcon, UserRound } from 'lucide-react'
...
export default function CustomerHome() {
  ...
  return (
    <main className="min-h-screen w-full overflow-x-hidden bg-[#f7f8fc] pb-24 text-slate-950">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 px-3 py-2.5 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur md:hidden"><div className="mx-auto flex max-w-2xl items-center justify-around gap-1"><span className="flex min-h-12 min-w-16 flex-col items-center justify-center gap-1 text-[12px] font-black text-blue-600"><Home className="h-5 w-5" /><span>Home</span></span><button onClick={() => chooseMode('all')} className="flex min-h-12 min-w-16 flex-col items-center justify-center gap-1 text-[12px] font-bold text-slate-500"><Grid2X2 className="h-5 w-5" /><span>Stores</span></button><Link to="/customer-orders" className="flex min-h-12 min-w-16 flex-col items-center justify-center gap-1 text-[12px] font-bold text-slate-500"><ClipboardList className="h-5 w-5" /><span>Orders</span></Link><Link to="/customer-orders" className="flex min-h-12 min-w-16 flex-col items-center justify-center gap-1 text-[12px] font-bold text-slate-500"><UserRound className="h-5 w-5" /><span>Account</span></Link></div></nav>
    </main>
  )
}
