...
class PublicStoreChatView(APIView):
    ...
    def post(self, request, slug):
        ...
        if not conversation:
            if not request.data.get('init_chat') and not request.data.get('text'):
                return Response({'detail': 'No conversation active.'}, status=status.HTTP_200_OK)
            ...
            # Re-link session_id to customer browser session
...
class PublicSendChatMessageView(APIView):
    ...
    def post(self, request, slug):
        ...
        conversation = get_object_or_404(ChatConversation, store=store, session_id=session_id)

        customer_name = request.data.get('customer_name') or conversation.customer_name
        customer_phone = request.data.get('customer_phone') or conversation.customer_phone
...
class SellerListConversationsView(APIView):
    ...
    def get(self, request, store_id):
        ...
        conversations = store.conversations.all().order_by('-last_message_at', '-created_at')
