...
function CustomerOrdersContent({ storeSlug }: { storeSlug: string }) {
  ...
  return <div className="mx-auto min-h-screen w-full max-w-md bg-slate-50 pb-32 lg:max-w-none lg:w-full">
    <header className="flex items-center justify-between bg-slate-950 px-5 py-5 text-white"><div><Link to={`/store/${storeSlug}`} className="text-xs font-semibold text-indigo-300">← Back to store</Link><h1 className="mt-1 text-xl font-bold">My Orders</h1></div><NotificationBellHeader /></header>
